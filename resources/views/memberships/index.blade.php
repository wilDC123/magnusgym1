...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        <a href="{{ route('memberships.create') }}" class="btn btn-success">Agregar Membresía</a>
    </div>
        ...
        <tbody>
            @foreach ($memberships as $membership)
                <tr>
                    ...
                    <td>{{ $membership->fecha_inicio }}</td>
                    <td>{{ $membership->fecha_fin }}</td>
                    <td>{{ $membership->dias_restantes }}</td>
                    <td>
